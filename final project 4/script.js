let togglemenu = document.getElementById('mobiletoggle');
            ...
            myData.addEventListener("submit",(e)=>{
                ...
                if(!emailsyndex.test(useremail.value.trim())){
                    document.getElementById("error-mail").textContent="please fill your correct mail id";
                    useremail.classList.add("error");
                    valid=false;
                }
                const phonePattern = /^[6-9]\d{9}$/;
                if(!phonePattern.test(usermobile.value.trim())){
                    document.getElementById("error-number").textContent="please fill correct mobile number";
                    usermobile.classList.add("error");
                    valid=false;
                }
                if(usercomments.value.trim().length >10){
                    document.getElementById("error-feed").textContent="please write max 10 words";
